...
from models.AbsensiMahasiswa import AbsensiMahasiswa
...
model = tf.keras.models.load_model("./utils/fixed model/model_FRHIMA_Gray_fix.h5")
label_encoder = np.load(
    "./utils/fixed model/labels_encoder_fix.npy", allow_pickle=True
).item()
...
async def delete_mahasiswa(nim: str, db: sqlite3.Connection = Depends(get_db)):
    cursor = db.cursor()
    cursor.execute("DELETE mahasiswa WHERE NIM = ?", (nim,))
    db.commit()
